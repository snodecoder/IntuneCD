...
def get_parser(include_help=True):
    ...
    parser.add_argument(
        "--enrich",
        help="If set, fetches Intune configurationSettings and configurationCategories for documentation enrichment",
        action="store_true",
    )
...
def start(args=None):
    ...
    def run_backup(
        path, output, exclude, token, prefix, append_id, max_workers, platforms, enrich
    ):
        results = []
